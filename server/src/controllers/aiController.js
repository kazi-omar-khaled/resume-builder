...
import { extractTextFromPdfBuffer } from "../utils/extractPdfText.js";

export const enhanceProfessionalSummary = async (req, res) => {
  try {
    if (!ai) {
      return res.status(500).json({ message: "AI service not configured (missing API keys)." });
    }
  ...
  try {
    ...
    if (!resumeText) {
      return res.status(400).json({
        message: req.file
          ? "Could not extract text from this PDF. If it was made in Canva or a design tool, export again with text selectable, or save from Word/Google Docs as PDF. Scanned image-only PDFs need OCR first."
          : "Missing resume file or text",
      });
    }
...
`;
    ...
    try {
      ...
      return res.status(400).json({ message: "Failed to parse AI output" });
    }

    // `candidateId` last so AI JSON cannot inject null and break unique index candidateId_1
